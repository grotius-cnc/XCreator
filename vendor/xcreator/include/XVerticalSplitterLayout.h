#ifndef XVERTICALSPLITTERLAYOUT_H
#define XVERTICALSPLITTERLAYOUT_H
...
class XVerticalSplitterLayout : public XWidget {
public:
    //! Empty constructor. setWindow or parent afterwards.
    XVerticalSplitterLayout(){}
    ...
    //! Type of this widget.
    std::string Type(){
        return "XVerticalSplitterLayout";
    }
    ...
    void draw(){
        ...
        myTopLayout->setSize({{myBackground->Size().Origin().X(),
                               myBackground->Size().Origin().Y(),
                               myBackground->Size().Origin().Z()},myBackground->Size().Width(),float((myRatio*myBackground->Size().Height())-(0.5*mySplitterWidth))});
        myTopLayout->setRelativeOriginOffset(mySize->RelativeOriginOffset());
        //! Only update size in height.
        myTopLayout->setRelativeSizeOffset({0,mySize->RelativeSizeOffset().second});
        myTopLayout->setColor({0.1,0.0,0.0,0.9});

        mySplitter->setSize({{myBackground->Size().Origin().X(),
                              myTopLayout->Size().Origin().Y()+myTopLayout->Size().Height(),
                              myBackground->Size().Origin().Z()},myBackground->Size().Width(),mySplitterWidth});
        ...
        myBottomLayout->setSize({{ myBackground->Size().Origin().X(),
                                   mySplitter->Size().Origin().Y()+mySplitter->Size().Height(),
                                   myBackground->Size().Origin().Z()},myBackground->Size().Width(),myBackground->Size().Height()-mySplitter->Size().Height()-myTopLayout->Size().Height()});
        myBottomLayout->setRelativeOriginOffset(mySize->RelativeOriginOffset());
        //! Only update size in height.
        myBottomLayout->setRelativeSizeOffset({0,mySize->RelativeSizeOffset().second});
        myBottomLayout->setColor({0.1,0.0,0.0,0.9});
        ...
        if(mySplitter->Event(XEventEnum::DraggedLeft)){
            float myPosition=Mouse.Position()->Y()-(myBackground->Size().Origin().Y());
            myRatio=myPosition/myBackground->Size().Height();
            if(myRatio<0){
                myRatio=0.0;
            }
            if(myRatio>1){
            ...
            // When ratio is 1. dont display right widget.
            myIsDragged=1;
        } else {
            myIsDragged=0;
        }
        ...
        if(mySplitter->Event(XEventEnum::Hovered) || mySplitter->Event(XEventEnum::DraggedLeft)){
            ...
            if((Mouse.Position()->X()>mySplitter->Size().Origin().X()+5) && (Mouse.Position()->X()<mySplitter->Size().Origin().X()+mySplitter->Size().Width()-5)){
                myCursor->setVResizeCursor();
            }
        }
        ...
        for(uint i=0; i<WidgetVec().size(); i++){
            if(i==0){
                WidgetVec().at(i)->setSize(myTopLayout->Size());
            }
            if(i==1){
                //! Prevent bad behaviour when the width=0;
                WidgetVec().at(i)->setSize(myBottomLayout->Size());
            }
        }
...
private:
    XString *myString=new XString("XVerticalSplitterLayout");
    ...
    *myTextColor=new XColor(1.0,1.0,1.0,0.9);
    ...
    XRectangular *myTopLayout=new XRectangular();
    XRectangular *myBottomLayout=new XRectangular();
