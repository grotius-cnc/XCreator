#ifndef XHORIZONTALLAYOUT_H
#define XHORIZONTALLAYOUT_H
...
public:
    ...
    void draw(){
        ...
        if(myStrechToParent){
            mySize->setSize(Parent()->ScissorSize());
        ...
        //! If this contains 2 widgets, remove the margin.
//        if(s==2){
//            x=mySize->Origin().X()+mySize->RelativeOriginOffset().X();
//            y=mySize->Origin().Y()+mySize->RelativeOriginOffset().Y();
//            z=mySize->Origin().Z();
//            w=mySize->Width()+mySize->RelativeSizeOffset().first;
//            h=mySize->Height()+mySize->RelativeSizeOffset().second;
//        } else {
            //! Draw content with margin.
...
//        }
        ...
        for(uint i=0; i<WidgetVec().size(); i++){
            WidgetVec().at(i)->setSize({{x,y,z},w/s,h});
            x+=w/s;
            WidgetVec().at(i)->draw();
        }
