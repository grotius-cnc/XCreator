#ifndef XSTRETCHLAYOUT_H
#define XSTRETCHLAYOUT_H
...
class XStretchLayout: public XWidget {
public:
    ...
    XStretchLayout(XWindow *theWindow){
    ...
    void draw(){
        ...
        //! Event cycle.
        if(Event(XEventEnum::PressedLeft)){}
        if(Event(XEventEnum::ReleasedLeft)){}
        //! When in edit mode, show drag.
        if(!Event(XEventEnum::DraggedLeft)){
            if(myToggleStretchToParent){
                mySize->setSize(ParentScissorSize());
            }
        }
        //! Event cycle.
        if(Event(XEventEnum::PressedMid)){
            myToggleStretchToParent=!myToggleStretchToParent;
        }
        ...
        for(uint i=0; i<WidgetVec().size(); i++){
            ...
            //! I expect most of time a XStretchLayout will contain just one child widget.
            WidgetVec().at(i)->setSize(myBackground->Size());
            WidgetVec().at(i)->draw();
        }
...
private:
    XString *myString=new XString("StretchLayout");
    ...
    *myTextColor=new XColor(1.0,1.0,1.0,0.9);
    ...
    bool myToggleStretchToParent=1;
};
#endif
