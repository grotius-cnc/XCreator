...
public:
    ...
    void run(){
        ...
        myFileOpen=new XFileOpen(Window());
        myFileOpen->setSize({{0,0,0},100,100});
        myFileOpen->init();
        addWidget(myFileOpen);
        ...
        while (!glfwWindowShouldClose(window)){
            ...
            myFileOpen->setSize(mySize->Size());
            ...
            //! If ok or cancel is pressed, the window may close. To avoid a eventual reload of the window a glfwWaitEvents is reqeusted.
            if(myFileOpen->ShouldClose()){
                std::cout<<"selected file result: "<<myFileOpen->Result()<<std::endl;

                glfwSetWindowShouldClose(window,1);
                //! Process the glfwSetWindowShouldClose event.
                glfwWaitEvents();
            };
...
private:
    GLFWwindow* window;
    std::string myDialogName="XFileOpen";
