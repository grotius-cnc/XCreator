...
public:
    ...
    void run(){
        ...
        myStream=new XTextureStream(Window());
        myStream->setSize({{10,30,0},400,400});
        addWidget(myStream);

        myButton=new XButton(Window());
        myButton->setSize({{0,0,0},120,25});
        addWidget(myButton);
        ...
        while (!glfwWindowShouldClose(window)){
            ...
            //! Wait until we got a image.
            XPixmap thePixmap=myServer->Pixmap();
            if(thePixmap.pixels.size()>0){
                myStream->setSize({{0,0,0},mySize->Width(),mySize->Height()});
                myStream->setTexture(thePixmap);
                glfwSetWindowSize(window,thePixmap.width,thePixmap.height);
            }
...
private:
    GLFWwindow* window;
    std::string myDialogName="XServer";
    int myWidth=500, myHeight=250;
    XSize *mySize=new XSize();

    XButton *myButton;
    XServer *myServer;
    XTextureStream *myStream;
};
#endif
